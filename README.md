...
- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
  ...
  <!-- actions --> 
    Actions are objects that have a type, and sometimes a payload. Created in an action creator function, or through thunk's dispatch function.

  <!-- reducer -->
    A reducer is a function tasked with updating the redux-store by replacing some or all aspects of it based on specific action cases.
...
- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?
    ...
    - Application state is housed in the store. Component state is housed in individual components. 
    - Application state is best utilized to store data that will be used by multiple components.
    - Component state is best used to store data used only by that particular component and, when applicable, its children.


- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

    A 'thunk' is a function returned by another function. 
    It allows us to perform asynchronous actions.
    It forces our action creators to utilize the dispatch function in order to create actions.
